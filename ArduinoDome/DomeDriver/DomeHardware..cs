...
//
// ASCOM Dome hardware class for NANO
//
...
namespace ASCOM.NANO.Dome
{
    ...
    internal static class DomeHardware
    {
        ...
        static DomeHardware()
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.NANO.Dome", MessageBoxButtons.OK, MessageBoxIcon.Error);
                throw;
            }
        }
        ...
        public static string Name
        {
            ...
            get
            {
                string name = "Arduino NANO Dome controller";
 ...
 //               Slewing = false;
   ...
   //                 Slewing = false;
            ...
            {
                LogMessage("AbortSlew", "get - not implemented");
                throw new PropertyNotImplementedException("AbortSlew", false);
            }
        ...
        internal static void CloseShutter()
        {
            if (!Connected) 
            { 
                string ReadShutter, readShutter, TestString = string.Empty;
                LocalServer.SharedResources.Connected = true;
                ReadShutter = LocalServer.SharedResources.SendMessage("CloseShutter#");
                readShutter = ReadShutter.Remove(ReadShutter.Length - 1, 1);
                if (readShutter == "ShutterClosing") {
                        Thread.Sleep(1000);
                        LogMessage("CloseShutter", "Shutter is closing");
                        domeShutterState = false;
              ...
              {
                LogMessage("CloseShutter", "get - not implemented");
                throw new PropertyNotImplementedException("CloseShutter", false);
              }
        LocalServer.SharedResources.Connected = false;
        }
 ...
 //       }
        ...
        internal static void OpenShutter()
        {
            if (!Connected)
            {
                string ReadShutter, readShutter, TestString = string.Empty;
                LocalServer.SharedResources.Connected = true;
                ReadShutter = LocalServer.SharedResources.SendMessage("OpenShutter#");
                readShutter = ReadShutter.Remove(ReadShutter.Length - 1, 1);

                if (readShutter == "ShutterOpening")
                {
                    Thread.Sleep(1000);
                    LogMessage("OpenShutter", "Shutter is opening");
                    domeShutterState = true;
                    LocalServer.SharedResources.SharedSerial.ClearBuffers();
                    LocalServer.SharedResources.Connected = false;
                    return;
                ...
                {
                    LogMessage("OpenShutter", "get - not implemented");
                    throw new PropertyNotImplementedException("OpenShutter", false);
                }
            LocalServer.SharedResources.Connected = false;
        }
